...
// historical_reporting_schema_migrations), so both functions are written
// against this minimal interface instead of depending on pgxpool.Pool
// directly.
...
// services/historical-reporting/cmd/main.go can reference it in
...
// Sourced from the SAME value services/historical-reporting/cmd/main.go
...
// design.
//
// historical_events itself carries no RLS policy (see this package's own
// doc comment -- no in-service tenant-scoped query path exists yet), but
...
// Historical Reporting's own queries need on Historical Reporting's own
// tables -- SELECT, INSERT, UPDATE, DELETE on historical_events, and this
// package's own historical_reporting_schema_migrations tracking table
// (the migration-check/record logic in Migrate above queries and writes
// it every run, so it needs the same grants as any other table this
// service's runtime pool touches).
//
...
func grantRuntimeRolePrivileges(ctx context.Context, exec pgExecutor) error {
	const tables = `historical_events, historical_reporting_schema_migrations`
	if _, err := exec.Exec(ctx, fmt.Sprintf(
