// Command historical-reporting is the scaffold entrypoint for the
// Historical Reporting & Analytics service (architecture doc Section 2.2):
// durably subscribes to all domain events via JetStream and materializes
// them for BI/SLA/compliance reporting. Never sits in the real-time path
// of any other service. Stateless ingestion, backed by a stateful
// analytical store.
//
// This scaffold starts a gRPC server with the shared tenant-context
// interceptors and standard health service registered (for operational
// health checks / future admin RPCs). No JetStream consumers or domain
// RPCs exist yet (future milestone).
package main
